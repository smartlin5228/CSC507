...
public class Login {
  ...
  /**  public static void addServiceRequestToDB(String roomservice, String servicetime, String roomnumber, String yname) throws SQLException {
        log("------------------------Local Login-----------------------------");
        
        String id="1";
        makeJDBCConnection();
        String defaultTableName = "userlogin";
        String insertQueryStatement = "insert into servicereq values (?,?,?,?,? )";
        userStat = mysqlConnection.prepareStatement(insertQueryStatement);
        userStat.setString(1,id);
        userStat.setString(2, roomservice);
        userStat.setString(3, servicetime);
        userStat.setString(4, roomnumber);
        userStat.setString(5, yname);
        userStat.executeUpdate();
        log(roomservice + " Added Successfully!");
        userStat.close();
        mysqlConnection.close();
    }
    **/
    
    public static void addServiceRequestToDB(ServiceRequest profile1) throws SQLException {
        log("------------------------Local Login-----------------------------");
     ...
     //   if(r==0) {
        	 insertQueryStatement = "insert into servicereq values (?,?,?,?,?)";
     //   }else {
        	
     //   		insertQueryStatement="update servicereq set fullname=? , DOB=? , email=? , address=? where ssn=? ";
     //   }
   //     int temid=getMaxIdFromCustomerDB()+1;
     //   String id=Integer.toString(temid);
        ...
        userStat = mysqlConnection.prepareStatement(insertQueryStatement);
        userStat.setString(1,id);
        userStat.setString(2, profile1.getroomservice());
        userStat.setString(3, profile1.getservicetime());
        userStat.setString(4, profile1.getroomnumber());
        userStat.setString(5, profile1.getyname());
        userStat.executeUpdate();
       
        log(profile1.getroomservice() +"for "+profile1.getroomnumber()+" Added Successfully!");
        userStat.close();
        mysqlConnection.close();
    }
